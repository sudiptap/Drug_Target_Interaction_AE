...
aucs = []
fold_idx = 1 
for train,test in kfold.split(input_tf_noisy):
    #print(train)
    model.fit(input_tf_noisy[train], input_tf_noisy[train], epochs=100, batch_size=10, verbose=0)
    scores = model.evaluate(input_tf_noisy[test], input_tf_noisy[test], verbose=0)
    decoded_img = model.predict(input_tf_noisy[test])
